...
- [ ] `T1.4` Consolidate dialog placement:
  ...
  - [x] Moved command-bar selection helper `relationshipSelection` to
  ...
  - [x] Moved settings dialog section `SettingsSection` to
...
- [ ] `T1.6` Keep compatibility shims at legacy dialog paths until final
  ...
  - [x] Added compatibility shim at
        `src/components/ui-new/dialogs/selections/relationshipSelection.ts`.
  - [x] Added compatibility shim at
...
- [ ] `T1.7` Update only UI-layer imports needed for this dialog/action move.
  ...
  - [x] Updated canonical callsites to
        ...
        `src/components/ui-new/dialogs/selections/ProjectSelectionDialog.tsx`.
  - [x] Updated canonical callsites to
        `@/features/command-bar/ui/dialogs/selections/relationshipSelection`
        in: `src/components/ui-new/dialogs/selections/ProjectSelectionDialog.tsx`.
  - [x] Updated canonical callsites to
  ...
  - [x] Kept non-owned `AssigneeSelectionDialog` callsite on the legacy shim
        path in `src/contexts/ActionsContext.tsx` to avoid Track 1 ownership
        churn.
  - [x] Updated canonical `WorkspaceSelectionDialog` callsite to
        feature-owned path in:
        `src/components/ui-new/containers/IssueWorkspacesSectionContainer.tsx`.
  - [x] Kept non-owned `WorkspaceSelectionDialog` callsite on the legacy shim
        path in `src/contexts/ActionsContext.tsx` to avoid Track 1 ownership
        churn.
  ...
  - [x] Kept non-owned `SettingsDialog` callsite on the legacy shim path in
        ...
        Track 1 ownership churn.
  - [x] Updated canonical `SettingsSection` callsite to feature-owned path in:
        `src/features/settings/ui/dialogs/SettingsDialog.tsx`.
...
- New information (2026-02-22): during staged `commandBar/*` helper
  extraction, moved feature-owned helpers can import not-yet-moved sibling
  helpers through legacy shim paths (for example
  `@/components/ui-new/dialogs/commandBar/injectSearchMatches`) to keep
  behavior stable until the sibling helper is migrated.
- New information (2026-02-22): `ProjectSelectionDialog` has active consumers
...
- New information (2026-02-22): after moving `WorkspaceSelectionDialog` into
  feature ownership, keep non-owned context callers (currently
  `src/contexts/ActionsContext.tsx`) on the legacy shim path while
  canonicalizing owned `src/components/ui-new/**` callsites.
...
- New information (2026-02-22): after moving `SettingsDialog` into feature
  ownership, keep settings submodules on legacy
...
- New information (2026-02-22): after moving `SettingsSection` into feature
  ownership, keep `SettingsDirtyContext` and section implementation modules
  (`GeneralSettingsSection`, `ReposSettingsSection`, `OrganizationsSettingsSection`,
  `RemoteProjectsSettingsSection`, `AgentsSettingsSection`, `McpSettingsSection`)
  on legacy `src/components/ui-new/dialogs/settings/*` paths for this track,
  with a compatibility shim at the legacy `SettingsSection` path.

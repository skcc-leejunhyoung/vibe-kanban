...
fn main() {
    ...
    // Create packages/web/dist directory if it doesn't exist
    let dist_path = Path::new("../../packages/web/dist");
    if !dist_path.exists() {
        println!("cargo:warning=Creating dummy packages/web/dist directory for compilation");
        fs::create_dir_all(dist_path).unwrap();

        // Create a dummy index.html
        let dummy_html = r#"<!DOCTYPE html>
<html><head><title>Build web app first</title></head>
<body><h1>Please build @vibe/web first</h1></body></html>"#;

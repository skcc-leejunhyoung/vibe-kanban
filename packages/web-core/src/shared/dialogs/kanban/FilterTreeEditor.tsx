...
// Shared editing context threaded to the (module-level, so their component
// identity is stable) recursive row/group renderers. Keeping the sub-components
// out of the parent body is what prevents a full remount — and therefore text
// input focus loss — on every keystroke, since each edit re-renders the tree.
type EditorContext = {
  root: AdvancedFilter;
  onChange: (next: AdvancedFilter) => void;
  statuses: FilterStatusOption[];
...
// --- Field / operator / value label helpers --------------------------------

const fieldLabel = (t: TFunction<'common'>, field: FilterFieldKey): string =>
  ({
...
): MultiSelectDropdownOption<string>[] => {
  const { statuses, tags, milestones, users, t } = ctx;
  switch (field) {
    case 'status':
    ...
    case 'priority':
      return PRIORITY_VALUES.map((priority) => ({
        value: priority,
        label: t(`kanban.priorities.${priority}`, PRIORITY_FALLBACK[priority]),
      }));
    case 'assignee':
      return [
        {
    ...
    <div className="flex flex-wrap items-center gap-half">
      ...
      {needsValues &&
        (node.field === 'text' ? (
          ...
          <MultiSelectDropdown
            values={node.values}
            options={valueOptions(ctx, node.field)}
      ...
      <button
        type="button"
        onClick={() =>
          onChange(removeNodeById(root, node.id) as AdvancedFilter)
        }
      ...
      <div className="mb-base flex flex-wrap items-center gap-half">
        ...
        <div className="ml-auto flex items-center gap-half">
          <button
            type="button"
            onClick={() =>
              onChange(addNodeToGroup(root, node.id, newCondition('status')))
            }
          ...
          <button
            type="button"
            onClick={() =>
              onChange(addNodeToGroup(root, node.id, newGroup('and')))
            }
            className="flex items-center gap-half rounded-sm bg-panel px-base py-half text-sm text-normal transition-colors hover:bg-secondary"
          >
      ...
      {node.children.length === 0 ? (
        ...
        >
          {node.children.map((child) =>
            renderNode(child, true, depth + 1, ctx)
          )}
        </div>
      )}
    </div>
...
const renderNode = (
  ...
  node.kind === 'group' ? (
    ...
    <ConditionRow key={node.id} node={node} ctx={ctx} />
  );
...
export function FilterTreeEditor({
  ...
  return (
    <div className="flex flex-col">{renderNode(value, false, 0, ctx)}</div>
  );
}

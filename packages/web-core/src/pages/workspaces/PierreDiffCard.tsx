...
interface PierreDiffCardProps {
  ...
  className: string;
  searchQuery?: string;
  onVisibleMatchCountChange?: (count: number) => void;
}
...
export function PierreDiffCard({
  ...
  const highlightKey = useMemo(
    () => `vk-search-highlight-diff-card:${filePath}`,
    [filePath]
  );
  ...
  useEffect(() => {
    ...
    const scheduleApply = () => {
      ...
      timeoutId = setTimeout(() => {
        ...
        rafId = requestAnimationFrame(applyHighlights);
      }, 60);
    };
  ...
  return (
    <div ref={cardRef} className={cn('pb-base rounded-sm', className)}>
      <div
        className={cn(

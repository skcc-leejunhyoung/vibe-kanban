...
lint_count() {
  ...
  (
    ...
    LINT_I18N=true npx --prefix "$REPO_ROOT/packages/web" eslint . \
      --ext ts,tsx \
      --format json \
      --output-file "$tmp" \
...
check_duplicate_json_keys() {
  local locales_dir="$REPO_ROOT/packages/web/src/i18n/locales"
  local exit_code=0
...
if (( PR_COUNT > BASE_COUNT )); then
  ...
  (cd "$REPO_ROOT/packages/web" && LINT_I18N=true npx eslint . --ext ts,tsx --rule "$RULE:error" -f codeframe 2>/dev/null || true)
  EXIT_STATUS=1

...
The changes panel shows a changed-file list on the left and the selected file's diff in the centre. If the file tree sidebar is open, changes from multi-repository workspaces are grouped by repository.

## 2. Select a changed file

The changed-file list shows all files that were added, modified, or deleted. Click any file to replace the diff in the viewer. In a multi-repository workspace, repository headings keep identical paths separate.

The optional file tree sidebar is also grouped by repository. Use its search box to filter files by name, or its toggle button to expand or collapse all folders.

...
| `link_workspace` | Link a workspace to a remote issue | `workspace_id`<br/>`issue_id` | None | Link confirmation |

### Workspace Sessions

| Tool | Purpose | Required Parameters | Optional Parameters | Returns |
|------|---------|-------------------|-------------------|---------|
| `start_workspace_session` | Start a new workspace session with a coding agent | `title`<br/>`executor`<br/>`repos` | `prompt_override` (alias: `prompt`)<br/>`variant`<br/>`issue_id` | Workspace ID |
...
When `issue_id` is provided, the workspace is automatically linked to the remote issue. If `prompt_override` is omitted, the linked issue's title and description are used as the workspace prompt.
...
4. **Start Workspace Sessions**: Use `start_workspace_session` to programmatically begin work on issues with specific coding agents
...
Each step uses the appropriate MCP tool (`list_organizations`, `list_projects`, `list_issues`, `create_issue`, `assign_issue`, `start_workspace_session`) to manage the complete workflow from planning to execution.

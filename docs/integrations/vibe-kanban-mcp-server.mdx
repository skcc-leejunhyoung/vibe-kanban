...
| `get_session_slice` | Read the conversation around a `search_sessions` hit | `session_id`<br/>`execution_id`<br/>`entry_index` | `radius` | Entries around the hit in session order, capped at ~64KB, with `truncated_before`/`truncated_after` |

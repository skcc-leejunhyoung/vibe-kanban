...
| `search_sessions` | Search past session conversations by content across all workspaces | `query` | `repo_id`<br/>`entry_types`<br/>`limit` | Ranked hits with session/workspace context and a snippet |
| `get_session_slice` | Read the conversation around a `search_sessions` hit | `session_id`<br/>`execution_id`<br/>`entry_index` | `radius` | Entries around the hit, in session order |
...
`search_sessions` matches substrings in indexed conversation entries (user prompts, assistant and thinking messages, tool-use summary lines, system and error messages; tool result payloads are not indexed). `entry_types` filters by entry kind, for example `["user_message", "tool_use"]`. Pass a hit's `session_id`, `execution_id` and `entry_index` to `get_session_slice` to read the surrounding conversation without loading the whole transcript.

...
For example, orchestrator mode can be launched with:

```json
{
  "mcpServers": {
    "vibe_kanban_orchestrator": {
      "command": "npx",
      "args": [
        "-y",
        "vibe-kanban@latest",
...
`--session-id` is required in orchestrator mode. If you only specify `--mcp`, the server defaults to `global` mode.

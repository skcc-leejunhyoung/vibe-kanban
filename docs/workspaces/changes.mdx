...
<Frame>
<img style={{maxHeight: "400px"}} src="/images/workspaces-changes-panel.png" alt="Changes panel showing the changed file list and side-by-side diff view" />
</Frame>
...
The changed files section shows every added, modified, renamed, copied, or deleted file:

- Select a file to display its diff.
- Use the status letter to identify how the file changed.
- Check the section header for the total number of changed files.
- In multi-repository workspaces, use the repository headings to identify where each file belongs.
...
The panel displays one selected file at a time. For a large diff, select **Load diff** to render it. If a file exceeds the inline diff size limit, select **Open in editor** to inspect it.

...
This removes eligible worktree directories, including their build outputs, but preserves database history and Git branches. It does not clean build caches in your original repository. Set `DISABLE_WORKTREE_CLEANUP` to disable inactive workspace cleanup; the development backend sets it by default.

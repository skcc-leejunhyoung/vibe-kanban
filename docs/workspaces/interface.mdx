...
Displays changed files beside the selected file's diff:

- **Changed-file list**: Select one added, modified, or deleted file at a time
- **Repository groups**: Keep changes separate in multi-repository workspaces
- **File tree sidebar**: Search and navigate changed files by repository and folder
- **Diff viewer**: See the selected file's changes with syntax highlighting
